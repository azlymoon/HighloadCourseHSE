...
| Отправка ответа     | Клиент идет в домен API, `Route 53` выбирает ближайший доступный ДЦ. Ответ попадает в `API-сервис`, затем в сервис прогресса. Перед записью проверяется `home_region`; состояние пользователя обновляется в своем региональном контуре через `PgBouncer -> PostgreSQL/Citus`, после чего инвалидируется `user_state_cache`. |
...
| **Итого**                  | -                         | -                                      | **58** | -                                                            |                                     - | без исходящего трафика  | **`$41 670.35`** |
